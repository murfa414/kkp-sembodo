...
class TransaksiImport implements ToCollection, WithHeadingRow
{
    ...
    public function __construct($filename = 'Data Import')
    {
    ...
    public function collection(Collection $rows)
    {
        ...
        foreach ($rows as $row) {
            ...
            elseif (!empty($row['no'])) {
                ...
                if ($finalArmada === null) {
                    foreach ($merkValid as $keyword) {
                        if (str_contains($rawDesc, $keyword)) {
                            // KETEMU DI DESKRIPSI!
                            if (str_contains($keyword, 'bus'))
                                $finalArmada = 'Bus Pariwisata';
                            elseif ($keyword == 'elf')
                                $finalArmada = 'Isuzu Elf';
                            elseif ($keyword == 'hiace')
                                $finalArmada = 'Toyota Hiace';
                            elseif ($keyword == 'innova')
                                $finalArmada = 'Toyota Innova';
                            elseif ($keyword == 'avanza')
                                $finalArmada = 'Toyota Avanza';
                            elseif ($keyword == 'xpander' || $keyword == 'expander')
                                $finalArmada = 'Mitsubishi Xpander';
                            elseif ($keyword == 'pajero')
                                $finalArmada = 'Mitsubishi Pajero';
                            else
                                $finalArmada = ucwords($keyword);
                            break;
                        }
                    }
                }
                ...
                // --- NORMALISASI NAMA KENDARAAN ---
                $kendaraanData['nama'] = $this->normalizeVehicleName($kendaraanData['nama']);
                ...
                Transaksi::create([
                    ...
                    'source_file' => $this->filename, // SIMPAN NAMA FILE
                    ...
                    'jenis_armada' => $kendaraanData['nama'],  // Nama yang sudah dinormalisasi
    ...
    /**
     * Normalisasi nama kendaraan agar konsisten
     * - Big Bus, Medium Bus, Bus → Bus Pariwisata
     * - Fortuner → Toyota Fortuner
     * - Alphard → Toyota Alphard
     * - Xpander → Mitsubishi Xpander
     * dll
     */
    private function normalizeVehicleName($name)
    {
        $nameLower = strtolower($name);

        // === 1. NORMALISASI BUS (gabungkan semua jenis bus) ===
        if (
            str_contains($nameLower, 'big bus') ||
            str_contains($nameLower, 'medium bus') ||
            str_contains($nameLower, 'micro bus') ||
            str_contains($nameLower, 'bus pariwisata') ||
            $nameLower === 'bus'
        ) {
            return 'Bus Pariwisata';
        }

        // === 2. NORMALISASI ELF (gabungkan semua varian Elf) ===
        if (str_contains($nameLower, 'elf')) {
            return 'Isuzu Elf 19 Seat';
        }

        // === 3. TAMBAHKAN MEREK JIKA BELUM ADA ===

        // Toyota models
        $toyotaModels = ['fortuner', 'innova', 'avanza', 'veloz', 'alphard', 'vellfire', 'camry', 'corolla', 'rush', 'agya', 'calya', 'hiace', 'voxy', 'yaris', 'raize'];
        foreach ($toyotaModels as $model) {
            if (str_contains($nameLower, $model) && !str_contains($nameLower, 'toyota')) {
                // Capitalize model name properly
                $name = preg_replace('/^(' . $model . ')/i', 'Toyota $1', $name);
        ...
        // Mitsubishi models
        $mitsubishiModels = ['xpander', 'expander', 'pajero', 'triton', 'outlander'];
        foreach ($mitsubishiModels as $model) {
            if (str_contains($nameLower, $model) && !str_contains($nameLower, 'mitsubishi')) {
                $name = preg_replace('/^(' . $model . ')/i', 'Mitsubishi $1', $name);
                $name = preg_replace('/\s+/', ' ', trim($name));
                // Fix typo expander -> xpander
                $name = str_ireplace('expander', 'Xpander', $name);
                break;
            }
        }

        // Honda models
        $hondaModels = ['civic', 'accord', 'brio', 'jazz', 'mobilio', 'brv', 'hrv', 'crv', 'city'];
        foreach ($hondaModels as $model) {
            if (str_contains($nameLower, $model) && !str_contains($nameLower, 'honda')) {
                $name = preg_replace('/^(' . $model . ')/i', 'Honda $1', $name);
                $name = preg_replace('/\s+/', ' ', trim($name));
                break;
            }
        }
        ...
        // Daihatsu models
        $daihatsuModels = ['xenia', 'terios', 'ayla', 'sigra', 'gran max', 'luxio'];
        foreach ($daihatsuModels as $model) {
            if (str_contains($nameLower, $model) && !str_contains($nameLower, 'daihatsu')) {
                $name = preg_replace('/^(' . preg_quote($model, '/') . ')/i', 'Daihatsu $1', $name);
                $name = preg_replace('/\s+/', ' ', trim($name));
                break;
            }
        }
        ...
        // Mercedes models
        if (
            (str_contains($nameLower, 'mercy') || str_contains($nameLower, 'mercedes') || str_contains($nameLower, 'benz'))
            && !str_contains($nameLower, 'mercedes benz')
        ) {
            $name = preg_replace('/(mercy|mercedes|benz)/i', 'Mercedes Benz', $name);
            $name = preg_replace('/Mercedes Benz Mercedes Benz/i', 'Mercedes Benz', $name);
            $name = preg_replace('/\s+/', ' ', trim($name));
        }

        // Suzuki models
        $suzukiModels = ['ertiga', 'xl7', 'jimny', 'baleno', 'ignis', 'sx4', 'karimun'];
        foreach ($suzukiModels as $model) {
            if (str_contains($nameLower, $model) && !str_contains($nameLower, 'suzuki')) {
                $name = preg_replace('/^(' . $model . ')/i', 'Suzuki $1', $name);
                $name = preg_replace('/\s+/', ' ', trim($name));
                break;
            }
        }
        ...
        // Hyundai models
        $hyundaiModels = ['stargazer', 'creta', 'palisade', 'ioniq', 'kona', 'santa fe'];
        foreach ($hyundaiModels as $model) {
            if (str_contains($nameLower, $model) && !str_contains($nameLower, 'hyundai')) {
                $name = preg_replace('/^(' . preg_quote($model, '/') . ')/i', 'Hyundai $1', $name);
                $name = preg_replace('/\s+/', ' ', trim($name));
                break;
            }
        }
        ...
        // Wuling models
        $wulingModels = ['almaz', 'cortez', 'confero', 'air ev', 'binguo'];
        foreach ($wulingModels as $model) {
            if (str_contains($nameLower, $model) && !str_contains($nameLower, 'wuling')) {
                $name = preg_replace('/^(' . preg_quote($model, '/') . ')/i', 'Wuling $1', $name);
                $name = preg_replace('/\s+/', ' ', trim($name));
                break;
            }
        }

        // === 4. NORMALISASI HIACE & COASTER ===
        if (str_contains($nameLower, 'hiace') || str_contains($nameLower, 'premio')) {
            if (!str_contains($nameLower, 'toyota')) {
                $name = 'Toyota ' . $name;
            }
        }

        if (str_contains($nameLower, 'coaster')) {
            if (!str_contains($nameLower, 'toyota')) {
                $name = 'Toyota Coaster';
            }
        }

        // === 5. NORMALISASI SPRINTER ===
        if (str_contains($nameLower, 'sprinter')) {
    ...
    private function detectLayanan($deskripsi, $jenisArmada)
    {
        ...
        if (
            str_contains($text, 'dengan driver') ||
